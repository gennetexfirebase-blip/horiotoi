...
export default async function ArticlePage({ params }: Props) {
  ...
  return <div className="article-page shell">
    ...
    <article className="article-layout">
      ...
      <div className="article-columns"><div className="article-main">{article.source === "cms" && article.image ? <div className="article-cover"><ArchiveImage src={article.image} alt={article.title} priority preserveRatio sizes="(max-width: 900px) 100vw, 760px" /></div> : null}<div className="article-content" dangerouslySetInnerHTML={{ __html: article.contentHtml }} />{article.tags.length ? <div className="tag-list"><span>Шошго</span>{article.tags.map((tag) => <Link key={tag} href={`/archive?q=${encodeURIComponent(tag)}`}>{tag}</Link>)}</div> : null}</div></div>
    </article>
